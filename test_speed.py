...
DISTANCE = 30  # <---- enter your distance-to-road value here
...
print("Image width in feet {} at {} from camera".format("%.0f" % frame_width_m, "%.0f" % DISTANCE))
...
for pic in pics:
        ...
        if state == WAITING:
            ...
            if state == TRACKING:
                if x >= last_x:
                    ...
                    abs_chg = initial_x - x
                kph = get_speed(abs_chg, mperpixel, secs)
    ...
    if True: #(state == WAITING):

        if motion_found:
            text_on_image = 'Vehicle velocity: {}'.format(last_kph)
            
        # draw the text and timestamp on the frame
        cv2.putText(image, 'Time: {}'.format(timestamp),
                    (10, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 1)
